...
DGE_DIR = '/wynton/home/oh/jhe3/MAST_FM/'

FCTHRESHOLD = log2(1.5)

# output 
# output_dir= '/wynton/home/oh/jhe3/MAST_FM/'
...
# Run for all datasets (h5ad objects)
...
dgeDataset <- function(input_dir_prefix = 'input_dir_prefix',
  ...
  # 3. DGE for either (1) All cells or (2) each cell type <- YJK (this could be cleaned up better)
  # if (AllCells==TRUE){
  #   sce_sub <-sce
  ...
  #   }
  #   write.csv(all_dge, file = paste0(output_dir, dataset,"_DGE_AllCellsCombined_" , data_id, ".csv") , quote = F, row.names = T)
  # }
  # else{
    for(c in 1:length(cell_type_list)){
      
      sce_sub <- splitAndProcess(sce, ann_labels = ann_col, cell_type = cell_type_list[c], min_counts = 2, min_cells = 5)
...
dge_MAST <- function(core = c(), covariate = "", condition = 'disease',
                     baseline_cond=c('1'),
                     contrast_cond=c('2')){
  # Export count matrix
  counts = assay(core)
...
merge_organs <- function(tissues = c('liver','lung','kidney','prostate','heart'), 
  ...
  if(write_out){
    write.csv(master_df, file = paste0(DGE_DIR, file_name, 'all_celltypes_DGE_',output_file_id,'_MAST.csv'), quote = F, row.names = F)
  }else{
    return(master_df)
  }
}